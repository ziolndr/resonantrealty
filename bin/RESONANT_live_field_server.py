...
class LiveFieldIndex:
    ...
    def display_row(self, rec: dict[str, Any], default_source: str) -> tuple[Any, ...]:
        ...
        original = metadata.get("original_record") if isinstance(metadata.get("original_record"), dict) else {}
        ...
        source_id = clean_text(pick(rec.get("source_id"), rec.get("sourceId"), original.get("id"), original.get("mlsId")))
        record_type = clean_text(pick(rec.get("type"), original.get("type"))) or ("paper" if source == "openalex" else "item")
        title = clean_text(pick(rec.get("title"), original.get("title"), original.get("address"))) or "Untitled"
        year = clean_text(pick(rec.get("year"), original.get("year"), original.get("yearBuilt")))
        external_url = clean_text(pick(rec.get("external_url"), rec.get("externalUrl"), original.get("url"), original.get("externalUrl")))

        publisher = clean_text(pick(rec.get("publisher"), metadata.get("publisher"), original.get("office"), original.get("sourceName"), original_meta.get("publisher")))
        availability = clean_text(pick(rec.get("availability"), metadata.get("availability"), original.get("status"), original_meta.get("availability")))

        genres = pick(rec.get("genres"), metadata.get("genres"), original.get("features"), original_meta.get("genres"), metadata.get("topics"), metadata.get("keywords"), [])
        if not isinstance(genres, list):
            genres = [genres] if genres else []
        genres = [clean_text(x) for x in genres if clean_text(x)][:12]
        ...
        overview = clean_text(pick(rec.get("overview"), original.get("remarks"), original.get("overview"), original.get("description")))
        if not overview:
        ...
        images = pick(original.get("images"), metadata.get("images"), original_meta.get("images"), [])
        if not isinstance(images, list):
            images = [images] if images else []
        images = [clean_text(x) for x in images if clean_text(x)][:40]
        image_url = clean_text(pick(rec.get("imageUrl"), metadata.get("imageUrl"), original.get("imageUrl"), original_meta.get("imageUrl"), images[0] if images else None))
        poster_path = clean_text(pick(rec.get("posterPath"), metadata.get("posterPath"), original.get("posterPath")))
        backdrop_path = clean_text(pick(rec.get("backdropPath"), metadata.get("backdropPath"), original.get("backdropPath")))
        tmdb_id = clean_text(pick(rec.get("tmdbId"), metadata.get("tmdbId")))
        tmdb_kind = clean_text(pick(rec.get("tmdbKind"), metadata.get("tmdbKind")))
        ...
        extra = {
            ...
            "squareFeet": original.get("squareFeet"),
            "lotSquareFeet": original.get("lotSquareFeet"),
            "yearBuilt": original.get("yearBuilt"),
            "propertyType": original.get("propertyType"),
            "neighborhood": original.get("neighborhood"),
            "features": original.get("features"),
            "images": images,
            "imageFallback": original.get("imageFallback"),
            "agent": original.get("agent"),
            "office": original.get("office"),
            "latitude": original.get("latitude"),
            "longitude": original.get("longitude"),
            "modified": original.get("modified"),
            "mlsId": original.get("mlsId"),
            "sourceName": original.get("sourceName"),
        }
    ...
    def search_vector(self, vector: Iterable[float], k: int, chunk_rows: int) -> list[dict[str, Any]]:
        info = self.manifest()
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    if args.html:
        print(f"[live-index] RESONANT: http://{args.host}:{args.port}/")
